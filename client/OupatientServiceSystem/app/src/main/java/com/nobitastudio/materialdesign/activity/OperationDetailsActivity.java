...
import com.nobitastudio.materialdesign.bean.ClientOperationData;
...
public class OperationDetailsActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    TextView operationRoomTextView;
    TextView operationAllPriceTextView;
    ...
    User user;
    ClientOperationData clientOperationData;
    ...
    private void init() {
        ...
        //init widget
        toolbar = findViewById(R.id.activity_operationdetails_toolbar);
        ...
        operationNameTextView = findViewById(R.id.activity_operationdetails_operationname_textview);
        patientNameTextView = findViewById(R.id.activity_operationdetails_patientname_textview);
        medicalCardNoTextView = findViewById(R.id.activity_operationdetails_medicalcardno_textview);
        priceTextView = findViewById(R.id.activity_operationdetails_price_textview);
        purchaseNumberTextView = findViewById(R.id.activity_operationdetails_purchasenumber_textview);
        startTimeTextView = findViewById(R.id.activity_operationdetails_starttime_textview);
        operationRoomTextView = findViewById(R.id.activity_operationdetails_operationroom_textview);
        operationAllPriceTextView = findViewById(R.id.activity_operationdetails_operationallprice_textview);
        orderNameTextView = findViewById(R.id.activity_operationdetails_ordername_textview);
        orderStateTextView = findViewById(R.id.activity_operationdetails_orderstate_textview);
        orderCreateTimeTextView = findViewById(R.id.activity_operationdetails_ordercreatetime_textview);
        orderPayMethodTextView = findViewById(R.id.activity_operationdetails_orderpaymethod_textview);
        orderSerialNoTextView = findViewById(R.id.activity_operationdetails_orderpayserialno_textview);
        doctorNoteTextView = findViewById(R.id.activity_operationdetails_doctornote_textview);
        payNowOrHavePayButton = findViewById(R.id.activity_operationdetails_paynoworhavepay_button);
        payNowOrHavePayCardView = findViewById(R.id.activity_operationdetails_paynoworhavepay_cardview);
        networkCacheCardView = findViewById(R.id.activity_operationdetails_networkcache_cardview);
        errorCardView = findViewById(R.id.activity_operationdetails_error_cardview);
        errorTextView = findViewById(R.id.activity_operationdetails_error_textview);
        payChoiceFrameLayout = findViewById(R.id.activity_operationdetails_paychioce_framelayout);
        cancelImageView = findViewById(R.id.activity_operationdetails_cancel_textview);
        aliPayImageView = findViewById(R.id.activity_operationdetails_alipay_imageview);
        vxPayImageView = findViewById(R.id.activity_operationdetails_vxpay_imageview);
        paySuccessCardView = findViewById(R.id.activity_operationdetails_paysuccess_cardview);
        ...
        //initClientListData
        initClientOperationData();

    }

    private void initBaseData() {
        user = AppDataUtil.getUser();
        operationCostId = AppDataUtil.getItemCostId();  //the orderid is operationCostId
        gson = new Gson();
    }

    private void initTestData() {
        userId = user.getUserAccount();
        tradeNo = operationCostId;
        tradeName = "药品单费";
    ...
    public void onClick(View v) {
        ...
        int viewId = v.getId();
        if (viewId == R.id.activity_operationdetails_paynoworhavepay_button) {
            // prepare to pay this order
            payChoiceFrameLayout.setVisibility(View.VISIBLE);
            payNowOrHavePayCardView.setVisibility(View.GONE);
        } else if (viewId == R.id.activity_operationdetails_cancel_textview) {
        ...
        } else if (viewId == R.id.activity_operationdetails_alipay_imageview) {
            // ali pay start
            payChoiceFrameLayout.setVisibility(View.GONE);
            callAliPayInterface(tradeName, tradeNo, cost, backParams, notifyUrl, userId);
        } else if (viewId == R.id.activity_operationdetails_vxpay_imageview) {
            // vx pay start
            payChoiceFrameLayout.setVisibility(View.GONE);
    ...
    private void initClientOperationData() {

        showNetworkCacheCardView();
        lockPayNowOrHavePayButton(GETTING_PAY_STATE);
        String requestAction = "getOperationCostDetails";
        String parameter = "&operationCostId=" + operationCostId;
        HttpUtil.sendOkHttpRequest(requestAction, parameter, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                clientOperationData = gson.fromJson(response.body().string(),ClientOperationData.class);
                closeNetworkCacheCardView();
                updateUi();
            }
        });
    ...
    private void updateUi() {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                //operation details
                operationNameTextView.setText(clientOperationData.getOperationName());
                patientNameTextView.setText(clientOperationData.getPatientName());
                medicalCardNoTextView.setText(String.valueOf(clientOperationData.getMedicalCardNo()));
                priceTextView.setText(String.valueOf(clientOperationData.getPrice()) + " 元");
                purchaseNumberTextView.setText(String.valueOf(clientOperationData.getPurchaseNumber()) + " 次");
                startTimeTextView.setText(Utility.getStartTime(clientOperationData.getStartTime()));
                operationRoomTextView.setText(clientOperationData.getOperationRoom());
                operationAllPriceTextView.setText(String.valueOf(clientOperationData.getPrice() * clientOperationData.getPurchaseNumber()));

                //order details
                Orders orders = clientOperationData.getOrders();
                ...
                //doctor note
                DoctorNote doctorNote = clientOperationData.getDoctorNote();
        ...
        TrPay.getInstance(this).callAlipay(tradeName, tradeNo, cost, backParams, notifyurl, userId, new PayResultListener() {
            ...
            public void onPayFinish(Context context, String outtradeno, int resultCode, String resultString, int payType, Long amount, String tradename) {
                if (resultCode == TrPayResult.RESULT_CODE_SUCC.getId()) {//1：支付成功回调
                    ...
                    userPaySuccess(clientOperationData.getOrders().getPaySerialNumber());
    ...
    private void userPaySuccess(String paySerialNumber) {
        ...
        confirmOperationCostOrder(paySerialNumber);
    }
     ...
     */
    private void confirmOperationCostOrder(String paySerialNumber) {
