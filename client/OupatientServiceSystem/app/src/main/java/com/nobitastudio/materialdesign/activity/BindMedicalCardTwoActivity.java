...
public class BindMedicalCardTwoActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    static final String BIND_MEDICALCARD_FAIL = "绑定失败,请重试";
    ...
    static final int MESSAGE_TYPE = 4; //0.注册账号 1.找回密码 2.修改密码 3.办理诊疗卡 4.绑定诊疗卡

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_bind_medical_card_two);
    ...
    private void init() {
        ...
        //init widget
        toolbar = findViewById(R.id.activity_bindmedicalcardtwo_toolbar);
        ...
        pictureVerificationCodeEditText = findViewById(R.id.activity_bindmedicalcardtwo_pictureverificationcode_edittext);
        pictureVerificationCodeImageView = findViewById(R.id.activity_bindmedicalcardtwo_pictureverificationcode_imageview);
        messageVerificationCodeEditText = findViewById(R.id.activity_bindmedicalcardtwo_messageverificationcode_edittext);
        getVerificationCodeButton = findViewById(R.id.activity_bindmedicalcardtwo_getmessageverificationcode_button);
        confirmBindButton = findViewById(R.id.activity_bindmedicalcardtwo_confirmbind_button);
        networkCacheCardView = findViewById(R.id.activity_bindmedicalcardtwo_networkcache_cardview);
        errorCardView = findViewById(R.id.activity_bindmedicalcardtwo_error_cardview);
        errorTextView = findViewById(R.id.activity_bindmedicalcardtwo_error_textview);
        sendSuccessCardView = findViewById(R.id.activity_bindmedicalcardtwo_sendsuccess_cardview);
        ...
        confirmBindButton.setOnClickListener(this);

    }


    private void initBaseData() {
        account = AppDataUtil.getUser().getUserAccount();
        waitForBindMedicalCard = AppDataUtil.getWaitForBindMedicalCard();
        ownerAccount = waitForBindMedicalCard.getOwnerAccount().toString();
        gson = new Gson();
    ...
    public void onClick(View v) {
        ...
        int viewId = v.getId();
        if (viewId == R.id.activity_bindmedicalcardtwo_getmessageverificationcode_button) {
            ...
            if (pictureVerificationCodeText.isEmpty()) {
                ...
                if (pictureVerificationCodeIsRight) {
                    ...
                    if (sendCounts < 6) {
                        ...
                        String parameter = "&account=" + ownerAccount
                                + "&messageType=" + MESSAGE_TYPE
                                + "&medicalCardNo=" + waitForBindMedicalCard.getMedicalCardNo()
                                + "&ownerName=" + waitForBindMedicalCard.getOwnerName();
        ...
        } else if (viewId == R.id.activity_bindmedicalcardtwo_confirmbind_button) {
            ...
            if (verificationCode.isEmpty()) {
                ...
                lockNextStepButton();
                String requestAction = "userBindMedicalCard";
                ...
                HttpUtil.sendOkHttpRequest(requestAction, parameter, new Callback() {
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        ...
                        message = gson.fromJson(response.body().string(), Message.class);
                        if (message.isSuccess()) {
                            AppDataUtil.getMedicalCards().add(waitForBindMedicalCard);
                            Utility.showToastLong(BindMedicalCardTwoActivity.this, "恭喜您已绑定成功~");
                            finish();
                        } else {
                            recoveryNextStepButton();
                            ...
                            } else if (message.getDescribe().equals("绑定失败")) {
                                showErrorCardView(BIND_MEDICALCARD_FAIL);
                            }
                        }
                    }
    ...
    private void lockNextStepButton() {
        confirmBindButton.setClickable(false);
        confirmBindButton.setBackgroundColor(getResources().getColor(R.color.colorGray));
    }
    ...
    private void recoveryNextStepButton() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                confirmBindButton.setClickable(true);
                confirmBindButton.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
            }
        });
    }
